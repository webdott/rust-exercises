...
impl <'a>SplitItems<'a> {
    fn new(split_str: &'a str, delimeter: char) -> Self {
        let (items, start, end) = split_str.char_indices().fold((vec![], None, None), |(mut res, mut start, mut end), (idx, curr)| {
            if curr == delimeter && start.is_some() && end.is_some() {
                res.push(&split_str[start.unwrap()..(end.unwrap() + 1)]);
                start = None;
                end = None;
            } else if curr != delimeter {
                if start.is_none() {
                    start = Some(idx);
                } 
...
impl <'a>Iterator for SplitItems<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let res = self.nth(self.idx);
        self.idx += 1;
        res
    }

    fn nth(&mut self, n: usize) -> Option<Self::Item> {
        (n < self.items.len()).then(|| self.items[n])
    }
}
