...
fn pc_transition(mut computer: ComputerState, event: Event) -> ComputerState {
    let is_running = computer.is_on && !computer.is_sleeping;
    let is_sleeping = computer.is_on && computer.is_sleeping;

    match event {
        Event::TurnOn => {
            computer = ComputerState {
                is_on: if !computer.is_on { true } else { computer.is_on },
                is_sleeping: if !computer.is_on { false } else { computer.is_sleeping },
                ..computer
            }
        },
        Event::TurnOff => {
        ...
        Event::MoveMouse => {
            computer = ComputerState {
                is_sleeping: if is_sleeping { false } else { computer.is_sleeping },
                idle_time: if is_running { 0 } else { computer.idle_time },
                sleep_time: if is_sleeping { 0 } else { computer.sleep_time },
                ..computer
            }
        },
        Event::PassTime(pass) => {
            computer = ComputerState {
                uptime: if computer.is_on { computer.uptime + pass } else { computer.uptime },
                idle_time: if is_running  { computer.idle_time + pass } else { computer.idle_time },
                sleep_time: if is_sleeping { computer.sleep_time + pass } else { computer.sleep_time },
                ..computer
            };

            if is_running && computer.idle_time > 1000 {
                computer.is_sleeping = true;
                computer.sleep_time = computer.idle_time - 1000;
                computer.idle_time = 0;
            }

            if computer.is_on && computer.is_sleeping && computer.sleep_time > 500 {
                computer = ComputerState::new_off();
            }
        }
    }
