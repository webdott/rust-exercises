...
// Compile regex once at program start
static TICKET_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"^(?<movie_name>[a-zA-Z0-9\s]+);(?<day_of_week>[a-zA-Z]+);?(?<visitor_name>[a-zA-Z0-9\s]*)$")
        .expect("Invalid regex pattern")
});

// Create days map once
static DAYS_MAP: Lazy<HashMap<&'static str, Day>> = Lazy::new(|| {
    HashMap::from([
        ("sunday", Day::Sunday),
        ("monday", Day::Monday),
        ("tuesday", Day::Tuesday),
...
fn parse_ticket(ticket: &str) -> Option<Ticket> {
    let captures = TICKET_REGEX.captures(ticket)?;
    
    let movie_name = captures.name("movie_name")?.as_str();
    
    let day_of_week = {
        let day_str = captures.name("day_of_week")?.as_str().to_lowercase();
        DAYS_MAP.get(day_str.as_str())?
    };
    
    let visitor = captures
        .name("visitor_name")
        .and_then(|m| {
            let s = m.as_str();
