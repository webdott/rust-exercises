...
impl EncryptedData {
    fn read(&mut self) -> Option<u8> {
        ...
        let data = self.bytes[self.pointer] ^ self.encrypted_key;
        self.pointer += 1;
        Some(data)
    }

    fn decrypt(self, key: u8) -> Result<Data, EncryptedData> {
...
impl Data {
    ...
    fn read(&mut self) -> Option<u8> {
        ...
        let data = self.bytes[self.pointer];
        self.pointer += 1;
        Some(data)
    }

    fn encrypt(self, key: u8) -> EncryptedData {
        EncryptedData {
            pointer: self.pointer,
            encrypted_key: key,
            bytes: self.bytes
