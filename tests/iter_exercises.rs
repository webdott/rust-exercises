...
fn find_third_42(vec: &[i64]) -> Option<usize> {
    vec.iter().enumerate().filter(|(_, ele)| **ele == 42).nth(2).map(|(idx, _)| idx)
}

fn keep_even<T>(into_iter: impl IntoIterator<Item=T>) -> impl Iterator<Item = T> {
    into_iter.into_iter().enumerate().filter_map(|(idx, ele)| (idx % 2 == 0).then_some(ele))
}
