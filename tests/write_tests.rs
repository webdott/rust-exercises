...
fn sanitize(input: &str) -> &str {
    ...
    if input.ends_with("x") {
        ...
        input = sanitize(input.trim_end_matches("x"));
    }

    if input.ends_with("o") {
        // Remove all o from the end of the string
        input = sanitize(input.trim_end_matches("o"));
    }

    // Remove .exe from the end
    if input.ends_with(".exe") {
        input = sanitize(&input[0..input.len() - 4]);
    }
...
mod tests {
    ...
    proptest! {
        #[test]
        fn test_sanitize_thoroughly(s in "[a-zA-Z0-9]*") {
            sanitize(&s);
        }
    }
}