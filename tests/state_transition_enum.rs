...
fn pc_transition(mut computer: ComputerState, event: Event) -> ComputerState {
    match event {
        ...
        Event::MoveMouse => {
            match computer {
                ComputerState::Sleeping {uptime, sleep_time} => ComputerState::Running {
                    uptime,
                    idle_time: 0,
                },
        ...
        Event::PassTime(pass) => {
            computer = match computer {
                ComputerState::Running {mut uptime, mut idle_time} => {
                    uptime += pass;
                    idle_time += pass;

                    if idle_time > 1000 {
                        let sleep_time  = idle_time - 1000;
                        computer = ComputerState::Sleeping {uptime, sleep_time};
                ...
                ComputerState::Sleeping {mut uptime, mut sleep_time} => {
                    uptime += pass;
                    sleep_time += pass;
