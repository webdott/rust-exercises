...
fn strip_prefix<'a>(needle: &'a str, prefix: &str) -> &'a str {
    let prefix_hash: HashSet<char> = prefix.chars().collect();
    
    let first_non_contained_pair = needle.char_indices().find(|(_, ch)| !prefix_hash.contains(ch));

    match first_non_contained_pair {
        Some((idx, _)) => &needle[idx..],
        None => ""
    }
}
