...
fn exercise_3(v: char) -> u8 {
    match v {
        x if Regex::new(r"[a-nA-N]").unwrap().is_match(&x.to_string()) => 1,
        y if Regex::new(r"[o-zO-Z]").unwrap().is_match(&y.to_string()) => 2,
        _ => 3
    }
}
...
fn exercise_4(v: char) -> bool {
    match v {
        x if Regex::new(r"[0-9]").unwrap().is_match(&x.to_string()) => true,
        _ => false
    }
}
...
fn exercise_5(v: char) -> Option<u32> {
    match v {
        x if Regex::new(r"[0-9]+").unwrap().is_match(&x.to_string()) => {
